...
class DQN:
    def __init__(self, env, cfg='./cfg/dqn_setup.json'):
        ...
        self.search_method = (data['search_method']).lower() \
            if 'search_method' in data.keys() else "epsilon"  # discount rate
        self.gamma = float(data['gamma']) if 'gamma' in data.keys() \
            else 0.95  # discount rate
        self.epsilon = float(data['epsilon']) if 'epsilon' in data.keys() \
            else 1.0  # exploration rate
        self.epsilon_min = float(data['epsilon_min']) \
            if 'epsilon_min' in data.keys() else 0.05
        self.epsilon_decay = float(data['epsilon_decay']) \
            if 'epsilon_decay' in data.keys() else 0.995
        self.learning_rate = float(data['learning_rate']) \
            if 'learning_rate' in data.keys() else 0.001
        self.batch_size = int(data['batch_size']) \
            if 'batch_size' in data.keys() else 32
        ...
        # Save infomation
        train_file_name = "dqn_mse_cartpole_%s_lr%s__tau%s_v1.log" \
            % (self.search_method, str(self.learning_rate), str(self.tau))
    ...
    def train(self):
        ...
        history = self.model.fit(batch_states, batch_target,
                                 epochs=1, verbose=0)
