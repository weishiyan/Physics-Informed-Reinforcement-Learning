    ...
    def fit(self, X_u, u, tf_epochs=5000, nt_config=lbfgs.Struct()):
        ...
        for epoch in range(tf_epochs):
            ...
            if epoch % 50 == 0:
                ...
                plt.xlabel("Time (s)")
                plt.ylabel("Theta")
                plt.title("%s Epochs" % str(epoch))
                plt.savefig("plots/%s_tf_Epochs.png" % str(epoch))
                plt.close()
            else:
                pass
            if tf.abs(loss_value - old_loss) < self.tolX:
                plt.clf()
                x = X_u
                exact = u
                predict = self.u_model(X_u)
                ...
                plt.savefig("plots/tf_Opt_Completed_%s_Epochs.png" % str(epoch))
                plt.close()
                break
            else: 
                pass
